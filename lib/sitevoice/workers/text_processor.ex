defmodule Sitevoice.Workers.TextProcessor do
  ...
  def perform(%Oban.Job{
    ...
    Logger.info("TextProcessor starting pipeline", log_id: log_id, org_id: org_id, attempt: attempt)

    result =
      :telemetry.span([:sitevoice, :pipeline], %{log_id: log_id, org_id: org_id}, fn ->
        {Reactor.run(Sitevoice.Reporting.Reactors.ProcessTextReport, %{
           log_id: log_id,
    ...
    case result do
      {:ok, _} ->
        Logger.info("TextProcessor pipeline completed")
        :ok

      {:error, reason} ->
        error_summary = format_pipeline_error(reason)
        Logger.error("TextProcessor pipeline failed: #{error_summary}")
  ...
  defp mark_failed(log_id, org_id, reason) do
    Logger.warning("TextProcessor marking log as failed: #{format_pipeline_error(reason)}")
    ...
    case Ash.get(Sitevoice.Reporting.DailyLog, log_id, authorize?: false, tenant: org_id) do
      {:ok, log} ->
        ...
        Logger.info("TextProcessor log marked as failed in database")

      {:error, fetch_err} ->
        Logger.error(
          "TextProcessor could not fetch log to mark as failed: #{inspect(fetch_err)}"
        )
    end
  end
