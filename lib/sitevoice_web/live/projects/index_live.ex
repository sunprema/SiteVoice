          ...
          <% else %>
            <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(280px, 1fr)); gap: 16px;">
              <%= for project <- @projects do %>
                <a href={~p"/projects/#{project.id}"} class="card card-link" style="height: 150px; display: flex; flex-direction: column; justify-content: space-between;">
                  <div>
                    <div style="font-family: var(--font-mono); font-size: 11px; color: var(--orange); letter-spacing: 2px; text-transform: uppercase; margin-bottom: 8px;">
                      <%= project.code %>
                    </div>
                    <div style="font-family: var(--font-display); font-size: 22px; letter-spacing: 0.5px; color: var(--white); line-height: 1.2;">
                      <%= project.name %>
                    </div>
                  </div>
                  <div style="font-size: 12px; color: var(--chalk); opacity: 0.45; font-family: var(--font-mono);">
                    <%= project.report_count || 0 %> submitted reports
                  </div>
                </a>
              <% end %>
            </div>
